...
module.exports = function (covServices) {
    ...
    function searchGames(req, rsp){
        ...
        covServices.searchGames(req.query.name)
            .then((games) => {
                sendSuccess(req,rsp,new Answer(`Displaying search results`,JSON.parse(games)),200)
            })
            .catch((err) => { 
                rsp.status(err.status).json(new Error(err.message, req.originalUrl))
            })
    }
  ...
  function editGroup(req, rsp){
      ...
      covServices.editGroup(groupId, req.body.newName, req.body.newDesc)
        .then((succ) => {
            sendSuccess(req,rsp,new Answer(`Group sucessfully edited.`,succ),200)
        })
        .catch((err) => {
            rsp.status(err.status).json(new Error(err.message, req.originalUrl))
        })
  ...
  function listGroups(req, rsp){
      covServices.listGroups()
        .then((groups) => {
            sendSuccess(req,rsp,new Answer(`Listing all groups`, groups),200)
        })
        .catch((err) => {
            rsp.status(err.status).json(new Error(err.message, req.originalUrl))
        })
  ...
  function showGroup(req, rsp){
      ...
      covServices.showGroup(groupId)
        .then((succ) => {
            sendSuccess(req,rsp,new Answer(`Displaying group ${succ.name}`, succ),200)
        })
        .catch((err) => {
            rsp.status(err.status).json(new Error(err.message, req.originalUrl))
        })
  ...
  function addToGroup(req, rsp){
      ...
      covServices.addToGroup(req.body.gameID, groupId)
        .then((res) => {
            sendSuccess(req,rsp,new Answer(`Game ${res.game} successfully added to group ${res.group.name}`, res.group),200)
        })
        .catch((err) => {
            rsp.status(err.status).json(new Error(err.message, req.originalUrl))
        })
  ...
  function removeFromGroup(req, rsp){
      ...
      covServices.removeFromGroup(groupId, req.body.gameID)
        .then((game) => {
            sendSuccess(req,rsp,new Answer("Game successfully removed", game),200) 
        })
        .catch((err) => {
            rsp.status(err.status).json(new Error(err.message, req.originalUrl))
        })
  ...
  function getGamesBetween(req, rsp){
      ...
      covServices.getGamesBetween(groupId, req.query.maxRating, req.query.minRating)
        .then((succ) => {
            sendSuccess(req,rsp,new Answer('Games that fit interval:',succ),200)
        })
        .catch((err) => { 
            rsp.status(err.status).json(new Error(err.message, req.originalUrl))
        })
